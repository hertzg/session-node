    ...
    return (req, res, parsedUrl) => {
        ...
        ;(() => {
            ...
            function errorListener (err) {
                Log.error('account-node-client: editProfile: ' + JSON.stringify(err))
                Error500Page(res)
            }
            ...
            var proxyReq = http.request({
                ...
                if (statusCode !== 200) {
                    Log.error('account-node-client: editProfile: HTTP status code ' + statusCode)
                    Error500Page(res)
                    return
                }
                ...
                proxyRes.on('end', () => {
                    ...
                    if (response !== true) {
                        Log.error('account-node-client: editProfile: Invalid response ' + JSON.stringify(response))
                        Error500Page(res)
                        return
                    }
